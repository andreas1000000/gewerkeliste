...
import { updateCompanyPremiumProfile } from "@/lib/actions/company-premium";
      ...
      <Shell>
        ...
        <form action={updateCompanyPremiumProfile} className="grid gap-6">
          ...
          <EditSection
            ...
            help="Eine Zeile pro Referenz: Titel | Projekttyp | Ort | Jahr oder Zeitraum | Beschreibung | Leistungen mit Komma | Herausforderung | Lösung | Kundentyp | Kundenname optional | Kunde öffentlich ja/nein | review_status"
            name="references"
            value={referencesText(premiumProfile.references)}
          />
          <EditSection
            title="Referenzbilder"
            help="Eine Zeile pro Bild: Referenztitel optional | Bildpfad/URL | Alt-Text | Bildunterschrift | review_status"
            name="reference_media"
            value={referenceMediaText(premiumProfile.referenceMedia, premiumProfile.references)}
          />
          <EditSection
          ...
          />
          <EditSection
            title="Social Media & weitere Kontaktwege"
            help="Im Basisprofil enthalten. Eine Zeile pro Link: Plattform | URL | Label optional | review_status"
            name="social_links"
            value={socialLinksText(premiumProfile.socialLinks)}
          />
...
function referencesText(items: Array<{
  title: string;
  ...
  return items.map((item) => [
    ...
    item.client_type,
    item.client_name,
    item.client_public ? "ja" : "",
    item.review_status,
  ].map(text).join(" | ")).join("\n");
}

function referenceMediaText(
  items: Array<{ reference_id: string | null; file_url: string; alt_text: string | null; caption: string | null; review_status: string }>,
  references: Array<{ id: string; title: string }>,
) {
  const titleById = new Map(references.map((reference) => [reference.id, reference.title]));
  return items.map((item) => [
    item.reference_id ? titleById.get(item.reference_id) || item.reference_id : "",
    item.file_url,
    item.alt_text,
    item.caption,
    item.review_status,
...
function socialLinksText(items: Array<{ platform: string; url: string; label: string | null; review_status: string }>) {
  return items.map((item) => [item.platform, item.url, item.label, item.review_status].map(text).join(" | ")).join("\n");
}
