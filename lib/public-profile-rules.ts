...
export function getPublicProfileEntitlements(company: EntitlementCompany): PublicProfileEntitlements {
  ...
  const verifiedStartEligible = profilePackage === "verified_start" && isVerified;
  ...
  const modules = {
    ...
    team: hasItems(premiumProfile.teamMembers),
    references: hasItems(premiumProfile.references) || Boolean(company.references_text),
    referenceMedia: hasItems(premiumProfile.referenceMedia),
    certificates:
    ...
    socialLinks: hasItems(premiumProfile.socialLinks),
    profileSections: hasItems(premiumProfile.profileSections),
  };
